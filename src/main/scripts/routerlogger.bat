...
IF "%1" == "" GOTO GUI
:CON
IF "%JAVA_HOME%" == "" java.exe -Xms4m -Xmx16m -classpath "%~dp0routerlogger.jar;%~dp0lib\*" it.albertus.router.RouterLogger %1 %2
IF NOT "%JAVA_HOME%" == "" "%JAVA_HOME%\bin\java.exe" -Xms4m -Xmx16m -classpath "%~dp0routerlogger.jar;%~dp0lib\*" it.albertus.router.RouterLogger %1 %2
GOTO END
:GUI
