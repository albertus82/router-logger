...
public class RootHtmlHandler extends BaseHtmlHandler {
	...
	public void service(HttpExchange exchange) throws IOException {
		...
		if (configuration.getBoolean(LogsHandler.CFG_KEY_ENABLED, LogsHandler.Defaults.ENABLED)) {
			html.append("<form action=\"").append(LogsHandler.PATH).append("\" method=\"").append(LogsHandler.METHODS[0]).append("\"><input type=\"submit\" value=\"").append(Messages.get("lbl.server.logs")).append("\" /></form>").append(NewLine.CRLF.toString());
		}
