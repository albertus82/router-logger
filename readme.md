...
* **`logger.hysteresis.ms`**= intervallo di tempo durante il quale l'applicazione continua a registrare a frequenza accelerata (`logger.interval.fast.ms`) anch dopo che un valore, che precedentemente ha raggiunto una soglia, è rientrato nella norma, in millisecondi (default: `10000` ms).
